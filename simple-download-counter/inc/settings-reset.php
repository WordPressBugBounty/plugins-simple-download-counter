	...
	if ($screen_id === 'sdc_download_page_download-counter-settings') {
		...
		if (!simple_download_counter_check_date_expired() && !simple_download_counter_dismiss_notice_check()) {
			...
			<div class="notice notice-success notice-lh">
				<p>
					<strong><?php esc_html_e('☀️ Summer Sale!', 'simple-download-counter'); ?></strong> 
					<?php esc_html_e('Take 35% OFF any of our', 'simple-download-counter'); ?> 
					...
					<?php esc_html_e('Apply code', 'simple-download-counter'); ?> <code>SUMMER</code> <?php esc_html_e('at checkout. Sale ends 9/20/2026.', 'simple-download-counter'); ?> 
...
function simple_download_counter_check_date_expired() {
	...
	$expires = apply_filters('simple_download_counter_check_date_expired', '2026-09-20');
